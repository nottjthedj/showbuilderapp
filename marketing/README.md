...
| [`_build_hub_page.py`](_build_hub_page.py) | **The production hub** — builds `production-hub.html`: the whole shoot in one page. Story, cast glossary, then every beat with its script, its clips, and its required b-roll, plus the shared element library. 188 generations, ticked off against one progress bar. The page to actually work from. |
...
**Generate ~2–2.5× what you cut.** 135 clips is 26 minutes of footage for a film that assembles to
**10m 53s** — a 2.41× ratio. That surplus is what lets you cut on the beat instead of using whatever
the model handed you.

**Batches run in film order.** Each character's clips are sorted by the master sheet's own sequence,
